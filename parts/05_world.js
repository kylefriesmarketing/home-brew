...
WORLD.addCollider = (x1,z1,x2,z2,id)=> WORLD.colliders.push({x1:Math.min(x1,x2),z1:Math.min(z1,z2),x2:Math.max(x1,x2),z2:Math.max(z1,z2),id});
WORLD.dropCollider = id => { WORLD.colliders=WORLD.colliders.filter(c=>c.id!==id); };
...
function buildBackdrop(scene){
  const mkRidge=(dist, h, col, op)=>{
    ...
    const m=new THREE.Mesh(new THREE.ShapeGeometry(shape),
      new THREE.MeshBasicMaterial({color:col, transparent:true, opacity:op, fog:false, depthWrite:false}));
  ...
  const treeAt=(x,z,kind)=>{
    const g=new THREE.Group(); const h=WORLD.getH(x,z);
    const s=rand(0.8,1.5);
    const trunk=new THREE.Mesh(trunkG, trunkM); trunk.position.y=0.7; trunk.castShadow=true; g.add(trunk);
    const top=new THREE.Mesh(kind==="s"?spruceG:blobG, kind==="s"?pick(sprM):pick(blobM));
    top.position.y = kind==="s" ? 3.2 : 3.0; top.castShadow=true; g.add(top);
    g.position.set(x,h,z); g.scale.set(s,s*rand(0.9,1.2),s);
